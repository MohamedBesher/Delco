...
namespace Saned.Delco.Admin.Controllers
{
    ...
    public class TripsController : BasicController
...

    {
        ...
        public PartialViewResult Search(RequestSearchModel search)
        {
            ...
            search.Type=RequestTypeEnum.Trip;
        ...
        private IQueryable<RequestListDto> SearchRequests(RequestSearchModel search)
        {
            if (!string.IsNullOrEmpty(search.UserId))
            {
                ...
                search.UserId = user!=null ? user.Id : "";
            }
            else          
                search.UserId = "";
            ...
            {
                var user = _repo.FindUserByName(search.AgentId);
                search.AgentId = user != null ? user.Id : "";
            }
            else
                search.AgentId = "";
        ...
        public JsonResult GetAgents(string text)
        {

            if (string.IsNullOrEmpty(text))
                return Json(null, JsonRequestBehavior.AllowGet);

            var agents = _repo.FindAllUsers(text, RolesEnum.Agent.ToString());
        ...
        public async Task<ActionResult> Cancel(RefuseRequestViewModel search)
        {
            try
            {
                ...
                if (request.Status!=RequestStatusEnum.Inprogress)
                    return RedirectToAction("InProgress", "Trips").Error("لا يمكنك الغاء هذا المشوار .");
                ...
                if (value != null)
                    await SendNotification(request, NotificationType.RequestCancele.GetHashCode(), "", value.ToString());
                else              
                    await SendNotification(request, NotificationType.RequestCancele.GetHashCode(), "", "");

             
                var message = string.Format("تم الغاء المشوار بنجاح .");
                return RedirectToAction("InProgress", "Trips").Success(message);
